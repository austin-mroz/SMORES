...
def _get_command_line_arguments() -> argparse.Namespace:
    ...
    parser.add_argument("catalyst_csv")
...
def get_core_smiles(path: str, catalyst_name: str,) -> str:
    catalyst_smiles = pd.read_csv(path)
    return catalyst_smiles[catalyst_smiles['catalyst_name'] == catalyst_name]['catalytic_smiles'].values[0]
...
def main() -> None:
    ...
    for calculation_directory in pathlib.Path(
        ...
        print(pathlib.PurePath(xyz_path).parts[-3])
        core_smiles = get_core_smiles(cli_args.catalyst_csv, r_group_name)
        print(core_smiles)
