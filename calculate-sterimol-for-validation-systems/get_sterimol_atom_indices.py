...
def main() -> None:
    ...
    for calculation_directory in pathlib.Path(cli_args.calculation_directory).rglob('geom.xyz'):
        ...
        groups,_ = rdRGroupDecomposition.RGroupDecompose([qcore],[full_mol],asSmiles=False,asRows=True)

        Draw.MolToFile(groups[0]['R1'], 'group.png')
        ...
        img = draw_it(full_mol, groups, range(0,1), qcore)
